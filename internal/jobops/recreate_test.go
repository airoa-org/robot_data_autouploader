...
func TestRecreateUploadJob(t *testing.T) {
	...
	// Recreate the job using shared function
	newJob, err := RecreateUploadJob(db, originalJob.ID)
	if err != nil {
...
func TestRecreateUploadJob_NonUploadJob(t *testing.T) {
	...
	_, err := RecreateUploadJob(db, copyJob.ID)
	if err == nil {
...
func TestRecreateUploadJob_JobNotFound(t *testing.T) {
	...
	_, err := RecreateUploadJob(db, "non-existent-id")
	if err == nil {
